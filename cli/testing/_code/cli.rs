...
fn module_inspection_preserves_invalid_governance_as_non_success() {
    ...
    fs::write(
        ...
        r#"{
          ...
          "logical_modules":[
            {"module":"AF-ZETA-0001","contract":"_data/logical_modules/zeta/contract.json","parent":"PF-FIXTURE","bindings":[{"kind":"directory","path":"src/zeta"}]},
            {"module":"AF-ALPHA-0001","contract":"_data/logical_modules/alpha/contract.json","parent":"PF-FIXTURE","bindings":[{"kind":"directory","path":"src/alpha"}]}
          ]
        }"#,
    )
...
impl SemanticCoverageFixture {
    ...
    fn with_policy(capability_allow: &[&str], capability_deny: &[&str], source: &str) -> Self {
        ...
        fs::write(
            root.join("sample/contract.json"),
            contract.to_canonical_json().expect("contract serializes"),
        )
        .expect("Module contract writes");
        fs::write(
            root.join("sample/_data/Cargo.toml"),
            "[package]\nname='coverage'\nversion='0.1.0'\nedition='2021'\n[lib]\npath='../_code/lib.rs'\n",
        )
        .expect("manifest writes");
        fs::write(root.join("sample/_code/lib.rs"), source).expect("source writes");
        Self { root }
    }
...
impl AuditFixture {
    ...
    fn new() -> Self {
        ...
        fs::create_dir_all(root.join("engine")).expect("engine Module creates");
        fs::write(
            root.join("engine/README.md"),
            module_readme("Fixture Engine"),
        )
        .expect("engine README writes");
        fs::write(
            root.join("engine/contract.json"),
            module_contract("AF-FIXTURE-ENGINE-0001", "Fixture Engine"),
        )
        .expect("engine contract writes");
        fs::create_dir_all(root.join("engine/_docs")).expect("engine docs creates");
        fs::write(
            root.join("engine/_docs/modules_docs.md"),
            modules_docs(&[
        ...
        for (module, identity, display) in [
            ...
            let module_root = root.join("engine").join(module);
            fs::create_dir_all(module_root.join("_code")).expect("Module code creates");
            fs::create_dir_all(module_root.join("_data")).expect("Module data creates");
            fs::create_dir_all(module_root.join("_docs")).expect("Module docs creates");
            ...
            .expect("Module contract writes");
            fs::write(module_root.join("_code/marker.txt"), "fixture")
                .expect("Module code marker writes");
            fs::write(
                module_root.join("_docs/code_docs.md"),
                code_docs(&["marker.txt"]),
            )
            .expect("Module code documentation writes");
        }
        for relative in [
            "engine/standard_registry/_data/standard_manifest.json",
            "engine/standard_registry/_data/std_id_rule.json",
            "engine/architecture_evaluation/_data/dependency_rule.json",
            "engine/architecture_evaluation/_data/realization_rule.json",
            "engine/architecture_evaluation/_data/semantic_conformance_rule.json",
            "engine/behavioral_semantics/_data/behavior_flow_rule.json",
            "engine/behavioral_realization/_data/behavior_bypass_rule.json",
            "engine/behavioral_realization/_data/behavior_realization_rule.json",
            "engine/semantic_analysis/_data/program_domain_rule.json",
            "engine/source_architecture/_data/source_artifact_rule.json",
            "engine/source_architecture/_data/source_profile_rule.json",
            "engine/state_effect_analysis/_data/program_state_rule.json",
            "engine/state_effect_analysis/_data/program_effect_rule.json",
            "engine/information_flow/_data/program_infoflow_rule.json",
            "engine/reference_resolution/_data/reference_rule.json",
            "engine/environmental_semantics/_data/program_environment_rule.json",
            "engine/environmental_semantics/_data/program_retry_rule.json",
            "engine/environmental_semantics/_data/program_recovery_rule.json",
            "engine/snapshot_governance/_data/ownership_rule.json",
            "engine/snapshot_governance/_data/traceability_rule.json",
            "engine/snapshot_governance/_data/test_boundary_rule.json",
            "engine/snapshot_governance/_data/module_rule.json",
            "engine/snapshot_governance/_data/documentation_rule.json",
            "engine/snapshot_governance/_data/contract_rule.json",
        ] {
            fs::copy(repository.join(relative), root.join(relative)).expect("standard file copies");
        }
        fs::write(
            root.join("engine/standard_registry/_docs/data_docs.md"),
            data_docs(&["standard_manifest.json", "std_id_rule.json"]),
        )
        .expect("standard Data documentation writes");
        fs::write(
            root.join("engine/architecture_evaluation/_docs/data_docs.md"),
            data_docs(&[
        ...
        fs::write(
            root.join("engine/behavioral_realization/_docs/data_docs.md"),
            data_docs(&[
        ...
        fs::write(
            root.join("engine/snapshot_governance/_docs/data_docs.md"),
            data_docs(&[
...
fn invalid_module_contract_explains_unevaluated_audit_and_module_inventory() {
    let fixture = AuditFixture::new();
    fs::write(fixture.root.join("engine/contract.json"), "{").expect("Module contract corrupts");
    ...
    assert!(
        audit_output.contains("engine/contract.json"),
        "{audit_output}"
    );
    assert!(
    ...
    assert!(
        module_output.contains("engine/contract.json"),
        "{module_output}"
    );
