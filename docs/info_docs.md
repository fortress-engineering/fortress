...
### [`source_artifact_model.json`](../info/source_artifact_model.json)

Persists Fortress's canonical language-neutral Source Artifact Model v1 so local and hosted gates can compare exact file inventory, responsibility, content identity, semantic-region coverage, profile/archetype status, lightweight semantic references, and structural conclusions without treating the absent Rust profile as failure or duplicating PSM bodies.
