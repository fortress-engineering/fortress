        ...
        .flat_map(|(index, (source, target))| {
            [1_u32, 2].map(|offset| {
                let evidence = ObservationProvenance::new(
                    format!("{}/_code/source.rs", source.to_ascii_lowercase()),
                    *source,
...
fn profiles_lca_and_testing_topology_are_recursive_and_distinct() {
    ...
    let ccg = compile([
        ("contract.json", root),
        ("engine/contract.json", engine),
        ("engine/provider/contract.json", provider),
        ("engine/testing/contract.json", testing),
    ]);
...
fn cross_scope_provider_collapses_consumers_and_preserves_lca_evidence() {
    ...
    let ccg = compile([
        ("contract.json", root),
        ("scope_a/contract.json", scope_a),
        ("scope_a/provider/contract.json", provider),
        ("scope_b/contract.json", scope_b),
        ("scope_b/consumer_one/contract.json", one),
        ("scope_b/consumer_two/contract.json", two),
    ]);
    let result = derive(
        &ccg,
        &observed(&[
...
fn narrower_and_isolated_diagnostics_respect_visibility_ancestry_and_testing() {
    ...
    let ccg = compile([
        ("contract.json", root),
        ("scope/contract.json", scope),
        ("scope/consumer_one/contract.json", consumer_one),
        ("scope/consumer_two/contract.json", consumer_two),
        ("narrow/contract.json", narrow),
        ("public/contract.json", public),
        ("public_isolated/contract.json", public_isolated),
        ("isolated/contract.json", isolated),
        ("tested/contract.json", testing_parent),
        ("tested/testing/contract.json", testing),
        ("ancestor/contract.json", ancestor),
        ("ancestor/descendant/contract.json", descendant),
    ]);
...
fn fragmented_authorized_surface_is_diagnostic_while_unauthorized_access_is_finding() {
    ...
    let ccg = compile([
        ("contract.json", root),
        ("foreign/contract.json", foreign),
        ("foreign/left/contract.json", left),
        ("foreign/right/contract.json", right),
        ("consumer/contract.json", consumer),
        ("one/contract.json", one),
        ("empty_foreign/contract.json", empty_foreign),
        ("empty_foreign/empty_left/contract.json", empty_left),
        ("empty_foreign/empty_right/contract.json", empty_right),
        ("empty_consumer/contract.json", empty_consumer),
    ]);
...
fn diagnostics_are_content_addressed_deterministic_and_epistemically_limited() {
    ...
    let ccg = compile([
        ("contract.json", root),
        ("isolated/contract.json", isolated),
        ("declared/contract.json", declared),
        ("consumer/contract.json", consumer),
    ]);
    let facts = observed(&[]);
