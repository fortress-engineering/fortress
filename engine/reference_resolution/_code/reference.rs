...
pub(crate) const REFERENCE_RULE_SOURCE: &str = include_str!("../_data/reference_rule.json");
...
impl ComponentResolutionIndex {
    ...
    pub fn preview_move(
        ...
        let destination = child_path(&parent.module_path, leaf);
...
pub fn evaluate_reference_resolution(
    ...
    let mut modules = ccg
        ...
        .map(|(id, module)| {
            ...
            let element_roots = ["_code", "_data", "_info"]
                .into_iter()
...
impl MarkdownInspection<'_> {
    ...
    fn inspect_cross_module(
        ...
        let child_catalog_projection = path.ends_with("_docs/modules_docs.md")
            && self
                .ccg
                .modules()
                .get(&target)
...
fn inspect_registry_boundaries(
    ...
    for (path, field, format) in [
        ...
        (
            "engine/standard_registry/_data/schema_manifest.json",
            "schemas",
            "schema-registry",
        ),
    ] {
...
impl RustInspection<'_> {
    ...
    fn inspect_occurrence(
        ...
        let crate_root_boundary = occurrence.kind == "path_attribute"
            && (path.ends_with("/_code/lib.rs")
                || path.ends_with("/_code/main.rs")
                || path == "_code/lib.rs"
                || path == "_code/main.rs");
...
fn is_governed_element_path(ccg: &ContractCoherencyGraph, path: &str, element: &str) -> bool {
    deepest_owner(ccg, path)
        ...
        .is_some_and(|module| {
            let root = child_path(module.path(), &format!("_{element}"));
    ...
    deepest_owner(ccg, path)
        ...
        .is_some_and(|module| {
            path == child_path(module.path(), "README.md")
                || path.starts_with(&format!("{}/", child_path(module.path(), "_docs")))
        })
}
