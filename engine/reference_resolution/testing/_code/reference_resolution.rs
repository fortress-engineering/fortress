...
fn fixture_files() -> BTreeMap<String, Vec<u8>> {
    ...
    BTreeMap::from([
        ...
        ("consumer/contract.json".into(), canonical(&consumer)),
        ("provider/contract.json".into(), canonical(&provider)),
    ])
}
...
fn synthetic_index(modules: usize, references: usize) -> ComponentResolutionIndex {
    ...
    for index in 0..modules {
        ...
        let path = if index < 10 {
            ...
            format!("m{:04}/m{index:04}", index % 10)
        };
...
fn stable_identity_resolves_to_portable_current_location() {
    ...
    files.insert("consumer/README.md".into(), b"# Consumer\n".to_vec());
    files.insert("provider/README.md".into(), b"# Provider\n".to_vec());
    ...
    assert_eq!(provider.module_path(), "provider");
    assert_eq!(provider.readme_path(), "provider/README.md");
...
fn machine_absolute_and_direct_cross_module_rust_paths_fail_exactly() {
    let mut files = fixture_files();
    files.insert(
        "consumer/README.md".into(),
        b"# Consumer\n\n## Relationships\n\n### [Provider](C:\\repo\\provider\\README.md)\n"
            .to_vec(),
    );
    files.insert(
        "consumer/_code/direct.rs".into(),
        b"include_str!(\"../../provider/_data/value.txt\");\n".to_vec(),
    );
    files.insert("provider/_data/value.txt".into(), b"value\n".to_vec());
    let ccg = fixture_ccg(&files);
    let result =
...
fn rule_allows_same_module_relative_path_and_rejects_cross_module_traversal() {
    let mut files = fixture_files();
    files.insert(
        "consumer/_code/use_data.rs".into(),
        b"const LOCAL: &str = include_str!(\"../_data/local.txt\");\nconst FOREIGN: &str = include_str!(\"../../provider/_data/value.txt\");\n".to_vec(),
    );
    files.insert("consumer/_data/local.txt".into(), b"local\n".to_vec());
    files.insert("provider/_data/value.txt".into(), b"foreign\n".to_vec());
    let ccg = fixture_ccg(&files);
    let result =
        evaluate_reference_resolution(&ccg, &files, "1.0.0-draft.1").expect("finding normalizes");
    assert_eq!(result.findings().len(), 1);
    assert!(result.index().references().iter().any(|reference| {
        reference.class() == ReferenceClass::Local
            && reference.projection() == Some("../_data/local.txt")
    }));
}
...
fn unknown_and_duplicate_stable_id_remain_ccg_failures() {
    ...
    let source = String::from_utf8(unknown["consumer/contract.json"].clone()).expect("UTF-8");
    unknown.insert(
        "consumer/contract.json".into(),
        source
    ...
    duplicate.insert(
        "duplicate/contract.json".into(),
        duplicate["provider/contract.json"].clone(),
    );
...
fn relative_navigation_is_portable_case_exact_and_location_sensitive() {
    assert_eq!(
        relative_navigation("a/README.md", "b/README.md"),
        "../b/README.md"
    );
    assert_eq!(
        relative_navigation("a/x/README.md", "a/README.md"),
        "../README.md"
    );
    assert_eq!(
        relative_navigation("README.md", "a/README.md"),
        "a/README.md"
    );
}
...
fn rust_native_package_identity_survives_module_relocation() {
    let mut files = fixture_files();
    files.insert(
        "consumer/_data/Cargo.toml".into(),
        b"[package]\nname = \"consumer-crate\"\nversion = \"0.1.0\"\n\n[dependencies]\nprovider-crate = { path = \"../../provider/_data\" }\n"
            .to_vec(),
    );
    files.insert(
        "provider/_data/Cargo.toml".into(),
        b"[package]\nname = \"provider-crate\"\nversion = \"0.1.0\"\n".to_vec(),
    );
    files.insert(
        "consumer/_code/lib.rs".into(),
    ...
    let preview = evaluation
        .index()
        .preview_move_to_path("AF-PROVIDER-0001", "platform/provider")
        .expect("move preview");
...
fn subtree_move_preserves_child_ids_and_all_semantic_references() {
    let modules = vec![
        ...
        ComponentResolution::new("AF-A-0001", "a", Some("PF-X".into())),
        ComponentResolution::new("AF-B-0001", "a/b", Some("AF-A-0001".into())),
        ComponentResolution::new("AF-C-0001", "a/b/c", Some("AF-B-0001".into())),
        ComponentResolution::new("AF-X-0001", "x", Some("PF-X".into())),
    ];
...
fn upward_and_downward_moves_only_affect_physical_boundaries() {
    let modules = vec![
        ComponentResolution::new("PF-X", "", None),
        ComponentResolution::new("AF-PAYMENTS-0001", "payments", Some("PF-X".into())),
        ComponentResolution::new(
            "AF-CURRENCY-0001",
            "payments/currency",
            Some("AF-PAYMENTS-0001".into()),
        ),
        ComponentResolution::new("AF-COMPILER-0001", "compiler", Some("PF-X".into())),
        ComponentResolution::new("AF-PARSER-0001", "parser", Some("PF-X".into())),
    ];
    ...
    let upward = index
        .preview_move_to_path("AF-CURRENCY-0001", "currency")
        .expect("upward move");
    let downward = index
...
fn genuine_semantic_dependency_change_remains_visible_beside_a_move() {
    let modules = vec![
        ComponentResolution::new("PF-X", "", None),
        ComponentResolution::new("AF-A-0001", "a", Some("PF-X".into())),
        ComponentResolution::new("AF-B-0001", "b", Some("PF-X".into())),
    ];
    ...
    let move_preview = before
        .preview_move_to_path("AF-B-0001", "platform/b")
        .expect("pure move");
