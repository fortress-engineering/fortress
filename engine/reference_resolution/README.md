...
`AF-FOO-0001` and `CAP-FOO` identify what an engineering entity is; `platform/foo` records where it is currently placed in the repository; a path such as `C:\\work\\fortress\\platform\\foo` is machine-local execution state and never persistent authority. A physical move preserves identity unless the authored contract intentionally changes semantics.
