...
pub(crate) const SOURCE_PROFILE_RULE_SOURCE: &str =
    include_str!("../_data/source_profile_rule.json");
pub(crate) const SOURCE_ARTIFACT_RULE_SOURCE: &str =
    include_str!("../_data/source_artifact_rule.json");
...
impl SourceProfileRegistry {
    ...
    pub fn standard() -> Self {
        Self::from_json_str(include_str!("../_data/source_profiles.json"))
...
pub fn evaluate_source_architecture(
    ...
    for entry in &source_inputs {
        ...
        let module_relative_path = semantic_source_location(&entry.owner_relative_path);
...
fn semantic_source_location(module_relative_path: &str) -> String {
    module_relative_path.strip_prefix("_code/").map_or_else(
        || module_relative_path.to_owned(),
        |relative| format!("code/{relative}"),
    )
}
