...
fn artifact_inputs(
    ...
    filing
        ...
        .map(|entry| {
            ...
            let relative = entry.path().rsplit_once("/_code/").map_or_else(
                || entry.path().strip_prefix("_code/").unwrap_or(entry.path()),
                |(_, value)| value,
            );
...
fn canonical_documentation_parser_supplies_authoritative_responsibility() {
    ...
    assert_eq!(responsibilities[0].source_path(), "_code/artifact.nom");
...
fn project_filing_ownership_and_content_identity_remain_separate() {
    ...
    changed.insert("_code/artifact.nom".into(), b"changed\n".to_vec());
    let second = evaluate(
        &changed,
        &SourceProfileRegistry::standard(),
        "nominal",
...
fn relocation_preserves_artifact_and_cross_module_semantic_identity() {
    fn moved_files(path: &str) -> BTreeMap<String, Vec<u8>> {
        BTreeMap::from([
            ...
            (format!("{path}/_code/unit.nom"), b"same bytes\n".to_vec()),
            (
                format!("{path}/_docs/code_docs.md"),
                b"# Code\n\n## Role\n\nRole.\n\n## Execution\n\nExecution.\n\n## State\n\nState.\n\n## Failure Semantics\n\nFailure.\n\n## Files\n\n### [`unit.nom`](../_code/unit.nom)\n\nStable responsibility.\n".to_vec(),
            ),
        ])
    }
    fn model(
        ...
        let source_path = files
            .keys()
            .find(|path| path.ends_with("/_code/unit.nom"))
            .expect("source");
    ...
    let before = model(&moved_files("old"));
    let after = model(&moved_files("new"));
    assert_eq!(
...
fn generated_source_is_code_and_requires_explicit_generator_provenance() {
    ...
    let missing = evaluate(
        ...
        "nom",
        &[],
        &[GeneratedSource::new("_code/artifact.nom", None)],
        &[],
        true,
        &["fixture-adapter"],
    );
...
fn ten_thousand_artifacts_remain_deterministic_and_lightweight() {
    let mut files = BTreeMap::from([
        ...
        ("_docs/code_docs.md".into(), b"# Code\n".to_vec()),
    ]);
    for index in 0..10_000 {
        files.insert(
            format!("_code/artifact_{index:05}.rs"),
