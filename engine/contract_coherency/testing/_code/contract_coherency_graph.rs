...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../contract_coherency/testing/_data")
}
        ...
        .map(|id| {
            ...
            let (path, classification, requirement) = declaration.map_or_else(
                ...
                |(contract_path, requirement)| {
                    ...
                    (
                        format!("{testing}/_code/fixture.rs"),
...
fn ecosystem_resolution_rejects_incoherent_intent() {
    ...
    assert_violation(
        &[
            ...
            ("provider/contract.json", descendant_ecosystem),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("duplicate/contract.json", duplicate_id),
        ],
        "duplicates",
    );
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", provider.clone()),
            ("second/contract.json", second_provider),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("consumer/contract.json", missing_provider),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", provider.clone()),
            ("consumer/contract.json", missing_capability),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", provider.clone()),
            ("consumer/contract.json", incompatible),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("verifier/contract.json", missing_target),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", provider.clone()),
            ("verifier/contract.json", unknown_subject),
        ],
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", unknown_constraint),
        ],
    ...
    assert_violation(
        &[
            ...
            ("provider/contract.json", redundant_child),
        ],
    ...
    let duplicates = resolve(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", feature_a),
            ("second/contract.json", feature_b),
        ],
        Some(&evidence),
    );
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", unknown_test),
        ],
    ...
    let valid_behavior = resolve(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", behavior.clone()),
            (
                "provider/testing/contract.json",
    ...
    assert_violation(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", unknown_feature),
        ],
    ...
    let cross = resolve(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", second_feature),
        ],
        Some(&cross_tests),
    );
    ...
    let direct_cycle = resolve(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", cycle_a),
            ("second/contract.json", cycle_b),
        ],
        None,
    );
    assert!(
    ...
    let transitive_cycle = resolve(
        &[
            ("contract.json", root.clone()),
            ("provider/contract.json", transitive_a),
            ("second/contract.json", transitive_b),
            ("third/contract.json", transitive_c),
        ],
        None,
    );
    assert!(
    ...
    let result = compile_contract_coherency_graph(
        &files(&[
            ...
            ("child/contract.json", inherited_child),
        ]),
        &ContractStandardIndex::from_bundle(&conflict_standard),
        None,
    );
    assert!(result.violations().iter().any(|violation| {
        violation.code() == "CCG-CONSTRAINT-CONFLICT" && violation.path() == "child/contract.json"
    }));
...
fn logical_contracts_join_the_same_ccg_identity_space() {
    ...
    let files = BTreeMap::from([
        ...
        (
            "_data/logical_modules/payments/contract.json".to_owned(),
            canonical(&logical).into_bytes(),
        ),
    ]);
    let sources = [LogicalModuleContractSource::new(
        "AF-PAYMENTS-0001",
        "_data/logical_modules/payments/contract.json",
        "PF-FIXTURE",
    )];
    let result =
...
fn physical_and_logical_contract_territories_resolve_without_path_inference() {
    ...
    let files = BTreeMap::from([
        ("contract.json".to_owned(), canonical(&root).into_bytes()),
        (
            "physical/contract.json".to_owned(),
            canonical(&physical).into_bytes(),
        ),
        (
            "_data/logical_modules/broad/contract.json".to_owned(),
            canonical(&broad).into_bytes(),
        ),
        (
            "_data/logical_modules/nested/contract.json".to_owned(),
    ...
    let sources = [
        LogicalModuleContractSource::with_directory_bindings(
            "AF-BROAD-0001",
            "_data/logical_modules/broad/contract.json",
            "PF-FIXTURE",
            ["crates", "vendor/broad"],
        ),
        LogicalModuleContractSource::with_directory_bindings(
            "AF-NESTED-0001",
            "_data/logical_modules/nested/contract.json",
            "PF-FIXTURE",
            ["crates/printer"],
        ),
    ];
    ...
    assert_eq!(
        resolver
            .resolve("_data/function_contracts.json")
            .expect("root physical contract resolves"),
        "PF-FIXTURE"
    );
    assert_eq!(
        resolver
            .resolve("physical/_data/state_contracts.json")
...
fn file_bindings_and_cargo_territories_cannot_author_distributed_contracts() {
    ...
    let files = BTreeMap::from([
        ("contract.json".to_owned(), canonical(&root).into_bytes()),
        (
            "_data/logical_modules/file/contract.json".to_owned(),
            canonical(&logical).into_bytes(),
        ),
    ]);
    let sources = [LogicalModuleContractSource::new(
        "AF-FILE-0001",
        "_data/logical_modules/file/contract.json",
        "PF-FIXTURE",
    )];
    let result =
        compile_contract_coherency_graph_with_logical_modules(&files, &standard(), None, &sources);
    ...
    assert!(matches!(
        resolver.resolve("legacy/_data/function_contracts.json"),
        Err(GovernanceTerritoryError::UnownedContract(_))
    ));
    assert!(matches!(
        resolver.resolve("crates/unmapped/_data/state_contracts.json"),
        Err(GovernanceTerritoryError::UnownedContract(_))
    ));
}
...
fn equal_physical_and_logical_territories_fail_deterministically() {
    ...
    let files = BTreeMap::from([
        ("contract.json".to_owned(), canonical(&root).into_bytes()),
        (
            "shared/contract.json".to_owned(),
            canonical(&physical).into_bytes(),
        ),
        (
            "_data/logical_modules/logical/contract.json".to_owned(),
            canonical(&logical).into_bytes(),
        ),
    ]);
    let sources = [LogicalModuleContractSource::with_directory_bindings(
        "AF-LOGICAL-0001",
        "_data/logical_modules/logical/contract.json",
        "PF-FIXTURE",
        ["shared"],
    )];
    let first =
...
fn logical_contract_ownership_survives_repository_relative_relocation() {
    ...
    let old = GovernanceTerritoryResolver::new(
        ...
        &[LogicalModuleContractSource::with_directory_bindings(
            module,
            "_data/logical_modules/relocatable/contract.json",
            "PF-FIXTURE",
            ["old/path"],
        )],
    )
    ...
    let logical = (0..1_000)
        .map(|index| {
            LogicalModuleContractSource::with_directory_bindings(
                ...
                format!("_data/logical_modules/{index:04}/contract.json"),
                "PF-FIXTURE",
...
/// `T-CANONICAL-MODULE-ATTEMPT-001`
/// Fortress classification: infrastructure
#[test]
fn malformed_direct_child_contract_remains_an_invalid_module_attempt() {
    let root = contract("PF-FIXTURE", "Fixture", true);
    let files = BTreeMap::from([
        ("contract.json".to_owned(), canonical(&root).into_bytes()),
        ("child/contract.json".to_owned(), b"{not-json".to_vec()),
        ("child/README.md".to_owned(), b"# Child\n".to_vec()),
        ("child/_code/lib.rs".to_owned(), Vec::new()),
    ]);
    let result = compile_contract_coherency_graph(&files, &standard(), None);
    assert!(!result.is_success());
    assert!(result.violations().iter().any(|violation| {
        violation.path() == "child/contract.json"
            && violation
                .message()
                .contains("Module Contract JSON is invalid")
    }));
}
