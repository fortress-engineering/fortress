    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidContractPath(path) => write!(
                formatter,
                "distributed contract path `{path}` is neither canonical `_data` nor logical territory `data`"
            ),
...
impl GovernanceTerritoryResolver {
    ...
    pub fn resolve(&self, path: &str) -> Result<&str, GovernanceTerritoryError> {
        let (territory_root, required_authority) = distributed_contract_territory(path)
        ...
        while let Some(candidate) = candidate_path {
            if let Some(territory) = self.by_path.get(candidate)
                && territory.authority == required_authority
                && (candidate == territory_root
                    || required_authority == GovernanceTerritoryAuthority::LogicalDirectoryBinding)
            {
    ...
    pub fn discover<'a>(
        ...
        let canonical_path = format!("_data/{file_name}");
        let canonical_suffix = format!("/{canonical_path}");
        let logical_path = format!("data/{file_name}");
        let logical_suffix = format!("/{logical_path}");
        let mut paths = paths
            .into_iter()
            .filter(|path| {
                *path == canonical_path
                    || path.ends_with(&canonical_suffix)
                    || *path == logical_path
                    || path.ends_with(&logical_suffix)
            })
            .collect::<Vec<_>>();
        paths.sort_unstable();
        paths.dedup();
...
impl<'a> Resolver<'a> {
    ...
    fn resolve(mut self) -> CcgCompilation {
        ...
        let module_index = CanonicalModuleIndex::from_paths(
            self.files.keys().map(String::as_str),
            |module, name| profiles.registered_directory(module, name),
        );
        ...
        let containment = derive_containment(&loaded, &id_to_path, &logical_parents, &module_index);
...
fn distributed_contract_territory(path: &str) -> Option<(&str, GovernanceTerritoryAuthority)> {
    ...
    if data_path == "_data" {
        return Some(("", GovernanceTerritoryAuthority::PhysicalModule));
    }
    if let Some(root) = data_path.strip_suffix("/_data") {
        return Some((root, GovernanceTerritoryAuthority::PhysicalModule));
    }
    if data_path == "data" {
        return Some(("", GovernanceTerritoryAuthority::LogicalDirectoryBinding));
    }
    data_path
        .strip_suffix("/data")
        .map(|root| (root, GovernanceTerritoryAuthority::LogicalDirectoryBinding))
}
