    ...
    for (id, module) in &graph.modules {
        ...
        if !module.contract().features().is_empty() && testing_id.is_none() {
            findings.push(semantic_violation(
                ...
                "/features",
                format!("Feature-owning Module `{id}` has no direct `testing` child"),
            ));
        }
        ...
        if module.path() == "tests"
            || module.path().ends_with("/tests")
            || module.path().ends_with("/unit_tests")
            || module.path().ends_with("/integration_tests")
            || module.path().ends_with("/e2e_tests")
        {
            findings.push(semantic_violation(
...
fn direct_testing_path(parent: &str) -> String {
    if parent.is_empty() {
        ...
        format!("{parent}/testing")
    }
}
fn is_testing_module(path: &str) -> bool {
    path == "testing" || path.ends_with("/testing")
}
fn testing_module_for_source(graph: &ContractCoherencyGraph, path: &str) -> Option<String> {
    graph
        ...
        .find(|(_, module)| {
            path.strip_prefix(&format!("{}/_code/", module.path()))
