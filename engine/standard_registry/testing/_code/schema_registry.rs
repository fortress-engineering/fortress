...
/// `T-AF-STANDARD-REGISTRY-0001-R03-015`
/// Fortress requirement: AF-STANDARD-REGISTRY-0001-R03
#[test]
fn installed_profiles_and_active_project_selection_match_registered_schemas() {
    let profile_schema =
        read_json("engine/standard_registry/_data/governance_profile_schema_v1.json");
    let profiles = read_json("engine/standard_registry/_data/governance_profiles_v1.json");
    jsonschema::draft202012::validate(&profile_schema, &profiles)
        .expect("installed profiles validate");

    let project_schema = read_json("engine/project_model/_data/project_schema_v4.json");
    let project = read_json("__fortress/.fsconfig");
    jsonschema::draft202012::validate(&project_schema, &project)
        .expect("active project profile selection validates");
}
