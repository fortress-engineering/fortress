...
fn data_path(name: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join(format!("../snapshot_governance/testing/_data/{name}"))
}
...
fn modules_docs(children: &[(&str, &str)]) -> Vec<u8> {
...
fn atomic() -> BTreeMap<String, Vec<u8>> {
    BTreeMap::from([
        ...
        ("_code/main.rs".into(), bytes("pub fn run() {}\n")),
        ("_docs/code_docs.md".into(), code_docs(&["main.rs"])),
    ])
}
...
fn add_child(files: &mut BTreeMap<String, Vec<u8>>, directory: &str, id: &str, display: &str) {
    ...
    files.insert(
        format!("{directory}/contract.json"),
        contract(id, display, &json!([])),
    );
    files.insert(
        format!("{directory}/_code/lib.rs"),
        bytes("pub fn child() {}\n"),
    );
    files.insert(
        format!("{directory}/_docs/code_docs.md"),
        code_docs(&["lib.rs"]),
    );
}
...
fn composite() -> BTreeMap<String, Vec<u8>> {
    let mut files = BTreeMap::from([
        ...
        (
            "_docs/modules_docs.md".into(),
            modules_docs(&[("child", "Child")]),
        ),
    ]);
...
fn all_elements() -> BTreeMap<String, Vec<u8>> {
    ...
    files.insert("_code/main.rs".into(), bytes("pub fn run() {}\n"));
    files.insert("_data/input.json".into(), bytes("{}\n"));
    files.insert("_info/result.json".into(), bytes("{}\n"));
    files.insert("_docs/code_docs.md".into(), code_docs(&["main.rs"]));
    files.insert("_docs/data_docs.md".into(), data_docs(&["input.json"]));
    files.insert("_docs/info_docs.md".into(), info_docs(&["result.json"]));
    files
}
...
fn relational(types: &[&str]) -> BTreeMap<String, Vec<u8>> {
    ...
    let mut files = BTreeMap::from([
        ...
        (
            "_docs/modules_docs.md".into(),
            modules_docs(&[("provider", "Provider")]),
        ),
    ]);
...
fn invalid_case(name: &str) -> BTreeMap<String, Vec<u8>> {
    ...
    match name {
        ...
        "missing_companion_docs" => {
            files.remove("_docs/code_docs.md");
        }
        "orphan_companion_docs" => {
            files.insert("_docs/data_docs.md".into(), data_docs(&["input.json"]));
        }
        "extra_file_under_docs" => {
            files.insert("_docs/notes.md".into(), bytes("# Notes\n"));
        }
        "subdirectory_under_docs" => {
            files.insert("_docs/archive/note.md".into(), bytes("# Note\n"));
        }
        ...
        "h4_heading" => {
            let source = String::from_utf8(files["_docs/code_docs.md"].clone()).expect("UTF-8");
            files.insert(
        ...
        "grandchild_cataloged_by_parent" => {
            files.insert(
                "child/_docs/modules_docs.md".into(),
                modules_docs(&[("grandchild", "Grandchild")]),
            );
            files.insert(
                "child/grandchild/README.md".into(),
                readme(
                    "Grandchild",
                    "This Module declares no outbound architectural relationships.",
                ),
            ...
            files.insert(
                "child/grandchild/contract.json".into(),
                contract("AF-GRANDCHILD-0001", "Grandchild", &json!([])),
            );
            files.insert(
                "child/grandchild/_code/lib.rs".into(),
                bytes("pub fn grandchild() {}\n"),
            );
            files.insert(
                "child/grandchild/_docs/code_docs.md".into(),
                code_docs(&["lib.rs"]),
            );
            replace(
                &mut files,
...
fn structured_data_catalog_documents_collection_without_leaf_explosion() {
    ...
    files.insert(
        "_data/dataset/historical_events_v3/part_000001/a.json".into(),
        bytes("{}\n"),
    );
    files.insert(
        "_data/dataset/historical_events_v3/part_000002/b.json".into(),
        bytes("{}\n"),
    );
    files.insert(
