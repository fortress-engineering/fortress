...
fn fortress_documentation_is_complete_synchronized_and_deterministic() {
    ...
    let files: BTreeMap<String, Vec<u8>> = observation
        ...
        .collect();
    let first = evaluate_documentation_files(&files, "1.0.0-draft.2")
        .expect("documentation evaluation completes");
    let second = evaluate_documentation_files(&files, "1.0.0-draft.2")
    ...
    assert_eq!(first.summary().modules_inspected(), 42);
    assert_eq!(first.summary().markdown_files_inspected(), 133);
    assert_eq!(
...
fn fortress_physical_tree_satisfies_recursive_module_grammar() {
    ...
    let paths: Vec<String> = observation
        ...
        .collect();
    let findings = evaluate_module_grammar(&paths, "1.0.0-draft.2")
