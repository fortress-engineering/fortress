...
fn equivalent_findings_have_identical_identity_and_json() {
    let first = finding("ARCH-DEPENDENCY-001", "engine/_code/lib.rs", "Violation.");
    let second = finding("ARCH-DEPENDENCY-001", "engine/_code/lib.rs", "Violation.");
    assert_eq!(first, second);
...
fn structured_analyzer_extracts_identity_classification_and_requirement() {
    let source = r"
        ...
        fn helper() {}
    ";
    let facts = analyze_rust_source("testing/_code/sample.rs", source).expect("source analyzes");
...
fn exact_draft_standard_documents_load_as_one_validated_bundle() {
    ...
    assert_eq!(bundle.edition(), "1.0.0-draft.2");
    assert_eq!(bundle.rules().len(), 23);
    assert!(matches!(
