...
pub(crate) const TEST_BOUNDARY_RULE_SOURCE: &str = include_str!("../_data/test_boundary_rule.json");
...
const REMEDIATION: &str = "Give every Feature-owning Module exactly one direct testing child, make that Testing contract verify exactly its immediate parent's local Features, keep canonical Testing Modules featureless, and place every Rust test directly beneath a canonical Testing Module's code directory.";
...
pub fn evaluate_testing_boundaries(
    ...
    Ok(TestingBoundaryEvaluation {
        ...
        testing_modules: ccg
            .modules()
            .values()
            .filter(|module| module.path() == "testing" || module.path().ends_with("/testing"))
            .count(),
