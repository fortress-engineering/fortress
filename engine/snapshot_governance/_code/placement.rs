...
pub(crate) const MODULE_RULE_SOURCE: &str = include_str!("../_data/module_rule.json");
...
const REMEDIATION: &str = "Converge the path to the canonical Project Filing System grammar: retain README.md and contract.json at every Module root; place lowercase snake_case child Modules directly beneath their parent; use only _code, _data, _docs, and _info for Fortress structure; keep project source files flat under _code; use the closed documentation set; and use the shallowest valid Data/Info role, collection, version, and partition structure.";
