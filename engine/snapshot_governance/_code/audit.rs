...
fn compile_source_architecture_from(
    ...
    evaluate_source_architecture(&SourceArchitectureInput {
        ...
        artifacts: &artifacts,
        project_model_authority: "project-model/project-filing-system-v2",
...
fn compile_psm_from_observed(
    ...
    let testing_modules = ccg
        .modules()
        .iter()
        .filter(|(_, module)| module.path() == "testing" || module.path().ends_with("/testing"))
        .map(|(id, _)| id.clone());
...
fn compile_analysis_source_architecture(
    ...
    evaluate_source_architecture(&SourceArchitectureInput {
        ...
        project_model_authority: if governed {
            "project-model/project-filing-system-v2"
        } else {
...
fn certification_verification_bindings(
    ...
    for (path, bytes) in files.iter().filter(|(path, _)| {
        path.as_str() == "_data/verification_bindings.json"
            || path.ends_with("/_data/verification_bindings.json")
    }) {
        ...
        if testing_path != "testing" && !testing_path.ends_with("/testing") {
...
fn build_affected_snapshot(
    ...
    let responsibility_units = prepared
        ...
        .keys()
        .filter(|path| {
            path.as_str() == "_docs/code_docs.md" || path.ends_with("/_docs/code_docs.md")
        })
...
fn load_standard(
    ...
    let Some(manifest_path) = manifest_path else {
        ...
        let manifest = LoadedDocument {
            path: "engine/standard_registry/_data/standard_manifest.json".into(),
