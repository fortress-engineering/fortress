...
use crate::module_index::{CanonicalModuleIndex, RESERVED_STRUCTURAL_DIRECTORIES, is_module_name};

/// Current filing profile schema identity.
pub const FILING_PROFILE_SCHEMA: &str = "urn:fortress:schema:v2:filing-system-profiles";
/// Current filing profile schema version.
pub const FILING_PROFILE_SCHEMA_VERSION: u16 = 2;
/// Current deterministic filing-model semantic version.
pub const FILING_MODEL_SEMANTIC_VERSION: &str = "2.0.0";
...
const CANONICAL_ELEMENTS: [(&str, &str); 4] = [
    ("code", "_code"),
    ("data", "_data"),
    ("docs", "_docs"),
    ("info", "_info"),
...
pub struct RegisteredRootEntry {
    ...
    scope: EcosystemEntryScope,
    opaque: bool,
}

/// Module-root scope at which an ecosystem registration applies.
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum EcosystemEntryScope {
    /// Apply only to the project Module root.
    ProjectRoot,
    /// Apply at every canonical Module root.
    ModuleRoot,
}
...
/// One explicit mechanical namespace tree permitted beneath `_code/`.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
#[serde(deny_unknown_fields)]
...
impl FilingSystemProfiles {
    ...
    pub fn standard() -> Self {
        Self::from_json_str(include_str!("../_data/filing_system_profiles.json"))
    ...
    fn validate(&self) -> Result<(), FilingProfileError> {
        ...
        for profile in &self.profiles {
            ...
            for entry in &profile.root_entries {
                ...
                if entry.path.starts_with('_') || entry.kind == RootEntryKind::File && entry.opaque
                {
                    return Err(FilingProfileError::InvalidRootEntry(
                        entry.path.clone().into(),
                    ));
            ...
            profile
                ...
                .find(|entry| {
                    entry.path == path
                        && entry.kind == kind
                        && match entry.scope {
                            EcosystemEntryScope::ProjectRoot => module.is_empty(),
                            EcosystemEntryScope::ModuleRoot => true,
                        }
                })
    ...
    pub(crate) fn registered_directory(&self, module: &str, path: &str) -> bool {
        self.entry_classification(module, path, RootEntryKind::Directory)
            .is_some()
    }
...
impl RegisteredRootEntry {
    ...
    pub fn new(
        ...
        Self {
            ...
            kind,
            classification,
            scope: EcosystemEntryScope::ProjectRoot,
            opaque: kind == RootEntryKind::Directory,
        }
    }

    /// Sets the explicit Module-root scope for this registration.
    #[must_use]
    pub const fn with_scope(mut self, scope: EcosystemEntryScope) -> Self {
        self.scope = scope;
        self
    }

    /// Sets whether a registered directory is opaque to Module indexing.
    #[must_use]
    pub const fn with_opacity(mut self, opaque: bool) -> Self {
        self.opaque = opaque;
        self
    }
}
...
impl MechanicalCodeStructure {
    /// Creates a mechanical namespace registration relative to `_code/`.
    #[must_use]
...
pub enum FilingViolationKind {
    ...
    /// `_docs/` contains a noncanonical file.
    UnrecognizedDocFile,
    /// `_docs/` contains a subdirectory.
    DocSubdirectoryForbidden,
    /// `_code/` contains unregistered project-semantic grouping.
    ...
    /// Module lacks README, contract, or implementation realization.
    MissingModuleRequirement,
    /// A leading-underscore Module-root entry is not Fortress structure.
    UnknownReservedEntry,
    /// A child Module directory is not lowercase snake case.
    InvalidModuleName,
    /// Direct Module-root names collide under case folding.
    CaseFoldCollision,
...
impl FilingViolationKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::UnknownReservedEntry => "UNKNOWN_RESERVED_ENTRY",
            Self::InvalidModuleName => "INVALID_MODULE_NAME",
            Self::CaseFoldCollision => "CASE_FOLD_COLLISION",
...
pub fn analyze_project_filing_system(
    ...
    let module_index =
        CanonicalModuleIndex::from_paths(files.iter().map(String::as_str), |module, name| {
            profiles.registered_directory(module, name)
        });
    let modules = module_index.paths();
    ...
    let module_records = modules
        ...
        .map(|module| FilingModule {
            ...
            elements: CANONICAL_ELEMENTS
                .iter()
                .filter(|(_, physical)| directories.contains(&child_path(module, physical)))
                .map(|(semantic, _)| (*semantic).to_owned())
                .collect(),
        })
        .collect();
    let inventory_entries = build_inventory(&files, &module_index);
    let inventory_digest = inventory_digest(&inventory_entries);
    let collections = build_collections(&inventory_entries);

    ProjectFilingModel {
        schema: "urn:fortress:model:v2:project-filing-system".into(),
        schema_version: 2,
        semantic_version: FILING_MODEL_SEMANTIC_VERSION.into(),
        root_entries: build_root_entries(&files, &directories, &modules, profiles),
...
fn evaluate_module(
    ...
    evaluate_module_root(module, files, directories, modules, profiles, violations);
    evaluate_module_realization(module, directories, modules, violations);
    ...
    evaluate_companion_docs(module, files, directories, modules, violations);
}
...
fn evaluate_module_root(
    ...
    evaluate_module_name(module, violations);
    ...
    let direct_directories = directories
        .iter()
        .filter(|directory| direct_parent(directory) == module)
        .collect::<Vec<_>>();
    ...
    for directory in direct_directories {
        ...
        if name.starts_with('_') {
            violation(
                violations,
                FilingViolationKind::UnknownReservedEntry,
                module,
                "module",
                directory,
                "all leading-underscore names are Fortress-reserved",
                "_code, _data, _docs, or _info",
            );
            continue;
        }
        if profiles
            .entry_classification(module, name, RootEntryKind::Directory)
            .is_some()
            || modules.contains(directory)
        {
            continue;
        }
        violation(
            ...
            "module",
            directory,
            "the closed Module-root precedence",
            "Fortress structure, applicable ecosystem structure, or a contract-marked child Module",
        );
    }
    for file in direct_files {
        ...
        if !matches!(name, "README.md" | "contract.json")
            && profiles
                .entry_classification(module, name, RootEntryKind::File)
                .is_none()
        {
            violation(
                violations,
...
fn evaluate_module_name(module: &str, violations: &mut Vec<FilingSystemViolation>) {
    if !module.is_empty() && !is_module_name(file_name(module)) {
        violation(
            violations,
            FilingViolationKind::InvalidModuleName,
            module,
            "module",
            module,
            "canonical child Module naming",
            "lowercase snake_case matching ^[a-z][a-z0-9]*(?:_[a-z0-9]+)*$",
        );
    }
}
...
fn evaluate_module_realization(
    ...
    let code = child_path(module, "_code");
    let has_code = directories.contains(&code);
    let has_child = modules
        .iter()
        .any(|candidate| direct_parent(candidate) == module && candidate != module);
    if !has_code && !has_child {
        violation(
            ...
            "_code/ for an atomic Module or at least one direct child Module",
        );
    }
}
...
fn evaluate_companion_docs(
    ...
    for (physical, document) in [
        ("_code", "code_docs.md"),
        ("_data", "data_docs.md"),
        ("_info", "info_docs.md"),
    ] {
        let element_path = child_path(module, physical);
        let document_path = child_path(&child_path(module, "_docs"), document);
        ...
        if element_exists && !document_exists {
            ...
            violation(
                ...
                &format!("remove _docs/{document} or add {physical}/"),
            );
        }
    }
    let modules_document = child_path(&child_path(module, "_docs"), "modules_docs.md");
    let has_children = modules
        .iter()
        .any(|candidate| direct_parent(candidate) == module && candidate != module);
    let has_document = files.contains(&modules_document);
    if has_children && !has_document {
        violation(
            violations,
            FilingViolationKind::MissingRequiredElementDoc,
            module,
            "docs",
            &modules_document,
            "every Module with child Modules has a canonical Modules document",
            "_docs/modules_docs.md",
        );
    } else if !has_children && has_document {
        violation(
            violations,
            FilingViolationKind::UnrecognizedDocFile,
            module,
            "docs",
            &modules_document,
            "the Modules document exists only when direct child Modules exist",
            "remove _docs/modules_docs.md or add a direct child Module",
        );
    }
}
...
fn evaluate_code(
    ...
    let root = child_path(module, "_code");
...
fn evaluate_docs(
    ...
    let root = child_path(module, "_docs");
    for directory in directories.iter().filter(|path| is_descendant(path, &root)) {
        violation(
            violations,
        ...
        .filter(|path| parent_path(path) == Some(root.as_str()))
    {
        if !CANONICAL_DOCS.contains(&file_name(file)) {
            violation(
                ...
                "code_docs.md, data_docs.md, info_docs.md, or modules_docs.md",
            );
        }
    }
}
...
fn evaluate_element(
    ...
    let root = child_path(module, physical);
...
fn build_root_entries(
    ...
    for path in files.iter().filter(|path| !path.contains('/')) {
        let classification = if matches!(path.as_str(), "README.md" | "contract.json") {
            ...
            profiles
                .entry_classification("", path, RootEntryKind::File)
                .unwrap_or(RootEntryClassification::Invalid)
        };
        entries.push(ClassifiedRootEntry {
            path: path.clone(),
...
fn build_inventory(
    files: &BTreeSet<String>,
    module_index: &CanonicalModuleIndex,
) -> Vec<FilingInventoryEntry> {
    files
        .iter()
        .map(|path| {
            let module = module_index.owning_module(path);
...
fn is_single_root_entry(path: &str) -> bool {
    !path.is_empty()
        ...
        && path != "."
        && path != ".."
        && !RESERVED_STRUCTURAL_DIRECTORIES.contains(&path)
        && !path.starts_with('_')
