...
use fortress_core::affected_analysis::{
    AffectedInput, AffectedSnapshot, AffectedUnit, AffectedUnitKind, AuthorityInputKind,
    IncrementalCacheState, IncrementalProjectionCache, InputChangeKind, ProjectionCacheKey,
    ProjectionDependency, ProjectionKind, analyze_affected, sha256,
};
...
fn independent_module_change_preserves_unrelated_reuse() {
    let previous = snapshot(
        "before",
        vec![input("a/_code/a.rs", "a0"), input("b/_code/b.rs", "b0")],
        vec![
    ...
    let current = snapshot(
        "after",
        vec![input("a/_code/a.rs", "a1"), input("b/_code/b.rs", "b0")],
        vec![
...
fn transitive_effect_change_invalidates_callers_not_independent_module() {
    let make = |operation: &str| {
        snapshot(
            operation,
            vec![input("b/_code/io.rs", operation)],
            vec![
                unit(
                    "authority:b",
                    AffectedUnitKind::AuthorityInput,
...
fn defeater_changes_invalidate_claim_and_evidence_dependents() {
    let make = |limitation: &str| {
        snapshot(
            limitation,
            vec![input("a/_code/lib.rs", limitation)],
            vec![
                unit(
                    "authority:a",
                    AffectedUnitKind::AuthorityInput,
...
fn contract_binding_and_function_authority_have_narrow_dependencies() {
    assert_eq!(
        input("native/data/function_contracts.json", "logical").kind(),
        AuthorityInputKind::FunctionContract
    );
    assert_eq!(
        input("native/data/state_contracts.json", "logical").kind(),
        AuthorityInputKind::StateContract
    );
    assert_eq!(
        input("native/data/environment_contracts.json", "logical").kind(),
        AuthorityInputKind::EnvironmentContract
    );
    assert_eq!(
        input("native/data/behavior_realization_contracts.json", "logical").kind(),
...
"#,
  ...
  "logical_modules": [{
    ...
    "contract": "_data/logical_modules/worker/contract.json",
...
"#
    ...
    let contract_path = root.join("_data/logical_modules/worker/contract.json");
...
"#,
    ...
    .expect("project configuration");
    fs::write(
        root.join("_data/logical_modules/worker/contract.json"),
        r#"{
  "$schema": "urn:fortress:schema:v2:module-contract",
  "schema_version": 2,
  "id": "AF-WORKER-0001",
...
"#,
    ...
    for (file_name, projection) in [
        ...
        let path = root.join("src/_data").join(file_name);
