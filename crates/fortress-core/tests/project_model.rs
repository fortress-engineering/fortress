...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("tests/fixtures/project-model")
}
...
fn valid_declared_project_loads() {
    let project = load("valid.json").expect("positive fixture must load");
...
fn duplicate_language_is_rejected() {
    let error = load("invalid-duplicate-language.json").expect_err("negative fixture must fail");
    assert!(matches!(
        error,
...
fn parent_traversal_path_is_rejected() {
    let error = load("boundary-parent-path.json").expect_err("boundary fixture must fail");
    assert!(matches!(
        error,
