...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("conformance/rules/STD-ID-001")
}
...
fn valid_fixture_passes() {
    let input = read_fixture("valid/identifier.txt");
...
fn invalid_fixture_produces_expected_rule_finding() {
    let input = read_fixture("invalid/identifier.txt");
...
fn shortest_registered_boundary_fixture_passes() {
    let input = read_fixture("boundary/identifier.txt");
    assert!(StableId::parse(input.trim()).is_ok());
}
