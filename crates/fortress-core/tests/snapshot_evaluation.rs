...
fn self_snapshot_and_standard() -> (RepositorySnapshot, StandardBundle) {
    ...
    let project_bytes = read_bytes(&root, ".fortress/project.json");
    ...
    let rule_text: Vec<(String, String)> = rule_sources
        ...
        .map(|(path, bytes)| {
            (
                path.strip_prefix("standard/drafts/1.0.0/")
                    .expect("rule is beneath standard root")
                    .to_owned(),
...
fn registered_architecture_evaluator_distinguishes_pass_and_failure() {
    ...
    let valid_source =
        fs::read_to_string(repository_root().join(".fortress/architecture/architecture.json"))
    ...
    let invalid_source = fs::read_to_string(
        repository_root().join("conformance/rules/ARCH-DEPENDENCY-001/invalid/architecture.json"),
    )
